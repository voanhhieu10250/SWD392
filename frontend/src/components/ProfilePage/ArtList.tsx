...
const fetchUserArts = async (pageParam: number, userId = '0') => {
  const res = await fetch(`${import.meta.env.VITE_API_ENDPOINT}/arts/user/${userId}?page=${pageParam}`)
...
const ArtList = () => {
  ...
  const { data, error, isError, fetchNextPage, hasNextPage, isFetching, isFetchingNextPage } = useInfiniteQuery<
    PageResponse<Art>,
    Error
  >(['profile-arts', userId], async ({ pageParam = 1 }) => await fetchUserArts(pageParam, userId), {
